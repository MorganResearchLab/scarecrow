...
<img style="float:right;width:100%;" src="../img/WTv2.svg" alt="scarecrow"/>
...
After indexing the reference genome, the FASTQ is aligned using 32 threads and 48G on a SLURM HPC. The resulting BAM is then sorted and index with samtools.
...
STAR --runThreadN ${SLURM_NTASKS} \
        --genomeDir ${GENOME_DIR} \
        --readFilesIn ${FASTQ} \
        --outFileNamePrefix ${PROJECT}/aligned/${ID%.fastq}. \
...
samtools sort \
    ...
    --write-index \
    -O BAM \
    -o ${PROJECT}/aligned/${ID%.fastq}.bam \
    ${PROJECT}/aligned/${ID%.fastq}.Aligned.out.bam

```

#### 8. Add barcode and UMI TAGS to BAM file

Adding read-specific TAGS to the BAM file poses some technical challenges given the large number of reads to cross-reference between the BAM and FASTQ file that contains the TAGS. To minimize RAM overhead we use an index of the FASTQ file - which is generated by the tool if it is absent. To avoid loading the FASTQ entirely into RAM in order to index it, the reads are processed in chunks and written to temporary files. Currently this step has not be written to support parallel processing, but there is the potential to work on this in the future. The temporary files are then merged using a heap and removed. Indexing a 53G FASTQ file in 1M read chunks takes ~ 30 mins. After indexing, the BAM file is split into *n = threads* chunks for parallel processing.
