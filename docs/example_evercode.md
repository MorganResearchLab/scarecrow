...
The above example uses the default set-based barcode matching method. The alternative is to use the trie-index method which is better suited to much larger barcode whitelists. The indexing method can be either kmer or seed based (see [pickle](./toolkit_pickle.md)). For comparison purposes, we run both methods here.
...
do
    mkdir -p ${PROJECT}/${METHOD}/barcode_profiles
    for BARCODE in ${BARCODES[@]}
    do
        scarecrow seed \
            --num_reads 10000 \
            ...
            --fastqs ${FASTQS[@]} \
            --barcodes ${BARCODE} \
            --pickle ${PROJECT}/${METHOD}/barcodes.${BARCODE%%:*}.pkl.gz \
            --index ${METHOD} \
            --out ${PROJECT}/${METHOD}/barcode_profiles/barcodes.${BARCODE%%:*}.csv
    done
done
```
...
Both the set- and trie-index methods are processed in the same manner with `harvest`. However, to illustrate that the same barcode profiles are generated, we can repeat the above on the trie-index method outputs from `seed`.

```bash
METHODS=('kmer' 'seed')
for METHOD in ${METHODS[@]}
do
    BARCODE_FILES=(${PROJECT}/${METHOD}/barcode_profiles/barcodes.*.csv)
    scarecrow harvest \
        ${BARCODE_FILES[@]} \
        --barcode_count 1 \
        --min_distance 10 \
        --conserved ${PROJECT}/${METHOD}/barcode_profiles/barcodes.${BARCODES[0]%%:*}_conserved.tsv \
        --out ${PROJECT}/${METHOD}/barcode_profiles/barcode_positions.csv
done
```
...
BC2:v1,2,SRR28867558_3.fastq.gz,forward,11,18,8852,0.93
BC3:v1,2,SRR28867558_3.fastq.gz,forward,49,56,8329,0.87
...
for METHOD in ${METHODS[@]}
do
    BARCODES=(BC1:n99_v5:${PROJECT}/${METHOD}/barcodes.BC1.pkl.gz
            BC2:v1:${PROJECT}/${METHOD}/barcodes.BC2.pkl.gz
            BC3:v1:${PROJECT}/${METHOD}/barcodes.BC3.pkl.gz)
    OUT=$(basename ${FASTQS[0]%.fastq*})
    mkdir -p ${PROJECT}/extracted/${METHOD}/J${JITTER}M${MISMATCH}
    sbatch -p uoa-compute --ntasks 1 --cpus-per-task ${THREADS} --mem 64G --time=24:00:00 -o reap.%j.out -e reap.%j.err \
        scarecrow reap \
            --threads ${THREADS} \
            --batch_size 20000 \
            --fastqs ${FASTQS[@]} \
            --barcode_positions ${PROJECT}/${METHOD}/barcode_profiles/barcode_positions.csv \
            --barcodes ${BARCODES[@]} \
            --extract 2:1-74 --umi 3:1-10 \
            --jitter ${JITTER} \
            --mismatch ${MISMATCH} \
            --out ${PROJECT}/extracted/${METHOD}/J${JITTER}M${MISMATCH}/${OUT} \
            --out_fastq
done
```
...
The Parse barcode whitelists have 96 (v1) and 99 (n99_v5) 8-mer barcodes. Given the small size of these whitelists the default set-based method is more efficient, as evident from the SLURM job logs summarised below based on using 16 threads.

| Method      | CPU Utilized | CPU Efficiency | Wall-clock time | Memory Utilized |
| ----------- | ------------ | -------------- | --------------- | --------------- |
| Set  (J0M2) |     02:07:31 |         21.86% |        00:36:27 |         2.68 GB |
| kmer (J0M2) |     11:20:41 |         77.19% |        00:55:07 |         2.42 GB |
| seed (J0M2) |     04:42:51 |         32.23% |        00:54:51 |         2.29 GB |
| Set  (J1M2) |     03:36:57 |         33.62% |        00:40:20 |         2.72 GB |
| kmer (J1M2) |   2-11:26:29 |         96.88% |        03:50:05 |         3.98 GB |
| seed (J1M2) |     12:07:40 |         83.83% |        00:54:15 |         2.46 GB |
| Set  (J2M2) |     04:12:36 |         40.62% |        00:38:52 |         2.72 GB |
| kmer (J2M2) |   4-09:19:27 |         98.16% |        06:42:22 |         4.51 GB |
| seed (J2M2) |     18:11:43 |         91.77% |        01:14:21 |         3.89 GB |
