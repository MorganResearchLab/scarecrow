...
#                        )
    ...
    def _get_sequence_with_jitter(
        ...
        # Handle cases where the end position accounting for jitter exceeds the sequence length
        if start -1 + barcode_length + jitter > len(full_sequence):
            for clip_count in range(1, jitter + 1):
                # Extract the original sequence clipped at the end (start is - 1 for Python 0-based position)
                original_seq = full_sequence[
                    (start - 1 + clip_count) : (start - 1 + barcode_length)
                ]
                # Insert 'N's at the end
                clipped_seq = original_seq + "N" * clip_count
                # if len(clipped_seq) == barcode_length:
                sequences.append(
                    (clipped_seq, start + clip_count)
                )  # -1 skips position 0
...
def validate_reap_args(parser, args) -> None:
    ...
    try:
        ...
        if not isinstance(args.out, str):
            raise TypeError("--out must be a string path prefix")
        ...
        # Validate barcode positions file
        if not isinstance(args.barcode_positions, str):
            raise TypeError("--barcode_positions must be a file path")
        if not os.path.exists(args.barcode_positions):
            raise ArgumentTypeError(f"Barcode positions file does not exist: {args.barcode_positions}")
        if not os.access(args.barcode_positions, os.R_OK):
            raise ArgumentTypeError(f"Barcode positions file is not readable: {args.barcode_positions}")
        if not args.barcode_positions.lower().endswith('.csv'):
            raise ArgumentTypeError(f"Barcode positions file should be a CSV file generated by scarecrow harvest: {args.barcode_positions}")
        ...
        if not isinstance(args.extract, str):
            raise TypeError("--extract must be a range string (e.g. '1:1-100')")
        if args.umi is not None and not isinstance(args.umi, str):
            raise TypeError("--umi must be a range string (e.g. '2:1-10') or None")

        # Validate barcode whitelists
        ...
        for barcode in args.barcodes:
            try:
                ...
                if not os.path.exists(file):
                    raise ArgumentTypeError(f"Barcode whitelist file does not exist: {file}")
                if not os.access(file, os.R_OK):
                    raise ArgumentTypeError(f"Barcode whitelist file is not readable: {file}")
            except ValueError:
                raise ArgumentTypeError(f"Barcode format should be 'name:version:file': {barcode}")

        # Validate boolean flags
        if not isinstance(args.out_sam, bool):
            raise TypeError("--out_sam must be a boolean")
        if not isinstance(args.out_fastq, bool):
            raise TypeError("--out_fastq must be a boolean")
        if not isinstance(args.barcode_reverse_order, bool):
            raise TypeError("--barcode_reverse_order must be a boolean")
        if not isinstance(args.gzip, bool):
            raise TypeError("--gzip must be a boolean")
        if not isinstance(args.verbose, bool):
            raise TypeError("--verbose must be a boolean")
        if not isinstance(args.sift, bool):
            raise TypeError("--sift must be a boolean")
        if not isinstance(args.scale_rna, bool):
            raise TypeError("--scale_rna must be a boolean")
        if not isinstance(args.evercode_wtv2, bool):
            raise TypeError("--evercode_wtv2 must be a boolean")

        # Validate mutually exclusive groups
        if args.out_sam and args.out_fastq:
            raise ArgumentTypeError("Cannot specify both --out_sam and --out_fastq")
        if args.scale_rna and args.evercode_wtv2:
            raise ArgumentTypeError("Cannot specify both --scale_rna and --evercode_wtv2")
