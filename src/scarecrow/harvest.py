...
def validate_harvest_args(parser, args):
    ...
    try:
        ...
        for barcode_file in args.barcodes:
            ...
            if not os.access(barcode_file, os.R_OK):
                raise ArgumentTypeError(f"Barcode file is not readable: {barcode_file}")
            if not barcode_file.lower().endswith('.csv'):
                raise ArgumentTypeError(f"Barcode file should be a CSV file generated by scarecrow seed: {barcode_file}")
        ...
        if not isinstance(args.barcode_count, int) or args.barcode_count <= 0:
            raise TypeError("--barcode_count must be a positive integer")
        if not isinstance(args.min_distance, int) or args.min_distance <= 0:
            raise TypeError("--min_distance must be a positive integer")

        # Validate conserved file if provided
        if args.conserved is not None:
            if not isinstance(args.conserved, str):
                raise TypeError("--conserved must be a string file path or None")
            if not os.path.exists(args.conserved):
                raise ArgumentTypeError(f"Conserved regions file does not exist: {args.conserved}")
            if not os.access(args.conserved, os.R_OK):
                raise ArgumentTypeError(f"Conserved regions file is not readable: {args.conserved}")
            if not args.conserved.lower().endswith('.tsv'):
                raise ArgumentTypeError(f"Conserved regions file should be a TSV file generated by scarecrow seed: {args.conserved}")
