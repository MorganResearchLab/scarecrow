...
def run_samstat(sam_file: str = None) -> None:
    ...
    if sam_file:
        if Path(sam_file).exists():
            ...
            names = ["CR_counts", "CB_counts", "XP_counts", "XM_counts", "UR_counts", 
                     "combined_CR_counts", "combined_CB_counts"]
...
def parse_sam_tags(sam_file: str = None):
    ...
    with pysam.AlignmentFile(sam_file, "rb", check_sq=False) as sam:
        ...
        return CR_counts, CB_counts, XP_counts, XM_counts, UR_counts, combined_CR_counts, combined_CB_counts
