...
class CompactTrie:
    def __init__(self, barcode_length: int):
        self.nodes = [CompactTrieNode()]  # Root node at index 0
        self.barcode_length = barcode_length  # Store the length of barcodes

    def _byte_to_index(self, byte):
        """Convert a byte to an index (A=0, C=1, G=2, T=3)."""
        return (byte >> 1) & 0b11  # Simple mapping for A, C, G, T

    def insert(self, sequence):
        """Insert a sequence into the trie."""
        node_index = 0  # Start at the root
        for byte in sequence.encode('ascii'):  # Convert to bytes
            idx = self._byte_to_index(byte)
            
            # Ensure the current node exists
            if node_index is None or node_index >= len(self.nodes):
                raise ValueError(f"Invalid node_index: {node_index}")
            ...
            if idx not in current_node.children:
                ...
                current_node.children[idx] = new_node_index

            # Move to the child node
            node_index = current_node.children[idx]

        # Mark the end of the sequence
        if node_index is not None and node_index < len(self.nodes):
            self.nodes[node_index].is_end = True
        else:
            raise ValueError(f"Invalid node_index at end of sequence: {node_index}")


    def find_matches(self, sequence):
        """
        Find all exact matches of barcodes in the sequence.
        Assumes all barcodes are of the same length (self.barcode_length).
        """
        matches = []
        sequence_length = len(sequence)

        # Iterate over all possible starting positions in the sequence
        for i in range(sequence_length - self.barcode_length + 1):
            node_index = 0  # Start at the root
            matched = True

            # Traverse the trie for the substring starting at position i
            for j in range(i, i + self.barcode_length):
                byte = sequence[j].encode('ascii')[0]  # Convert to byte
                idx = self._byte_to_index(byte)
                if idx in self.nodes[node_index].children:
                    node_index = self.nodes[node_index].children[idx]
                else:
                    matched = False
                    break

            # If a match is found, add it to the results
            if matched and self.nodes[node_index].is_end:
                start = i
                end = i + self.barcode_length
                matched_barcode = sequence[start:end]
                matches.append((start, end, matched_barcode))
...
class BarcodeMatcherTrie(BarcodeMatcher):
    ...
    def report_memory_usage(self):
        ...
        if hasattr(self, 'kmer_index') and self.kmer_index:
            for whitelist_key, kmer_index in self.kmer_index.items():
                kmer_index_memory += asizeof.asizeof(kmer_index)
    ...
    def _build_trie(self, barcode_sequences: Dict[str, Set[str]]):
        """
        Constructs a custom trie from barcode sequences
        """
        ...
        first_barcode = next(iter(next(iter(barcode_sequences.values()))))
        self.barcode_length = len(first_barcode)
        self.logger.info(f"Barcode length: {self.barcode_length}")

        for whitelist_key, sequences in barcode_sequences.items():
            # Initialize a custom trie for this whitelist
            custom_trie = CompactTrie(self.barcode_length)
    ...
    def _save_trie(self, pickle_file: str):
        ...
        try:
            # Prepare trie for saving
            automata_data = {}
            for whitelist_key, trie in self.automata.items():
                automata_data[whitelist_key] = {
                    'nodes': trie.nodes,
                    'barcode_length': trie.barcode_length
                }
            
            # Prepare kmer index for saving
            kmer_index_data = {}
            for whitelist_key, kmer_index in self.kmer_index.items():
                kmer_index_data[whitelist_key] = {
                    'kmer_to_barcode_indices': kmer_index.kmer_to_barcode_indices,
                    'barcodes': kmer_index.barcodes,
                    'kmer_length': kmer_index.k
                }
...
def _output_trie_node(trie, node_index, prefix, max_depth, depth):
    ...
    for idx, child_index in enumerate(node.children):
        if child_index is not None:
            first_child_index = child_index
            byte_value = int_to_base(idx)  # Convert index to base (A, C, G, T)
            break
    
    if first_child_index is not None:
        print(prefix + "└── " + f"{byte_value} (depth {depth})")
    else:
        print(prefix + "└── End (depth {depth})")
    ...
    if depth >= max_depth - 1:
        ...
        for idx, child_index in enumerate(node.children):
            if child_index is not None:
                ...
                child_byte_value = int_to_base(idx)  # Convert index to base (A, C, G, T)
    ...
    if first_child_index is not None:
        ...
        num_children = sum(1 for child in node.children if child is not None)
        if num_children > 1:
            print(new_prefix + f"├── ... ({num_children - 1} more children)")


def int_to_base(index):
    """Convert an integer index to its corresponding base (A, C, G, T)."""
    bases = ['A', 'C', 'G', 'T']
    return bases[index]
