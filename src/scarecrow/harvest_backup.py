...
def plot_peaks_faceted(barcode_data, save_fig=True, outfile='plot_faceted.png', dpi:int = 300):
    ...
    def plot_facet(data, ylim = None):
        g = sns.FacetGrid(data, col = "read", row = "barcode_whitelist", hue = "read", 
        ...
        if ylim is not None:
            g.set(ylim = ylim)  # Set the same y-axis limit for all facets
    ...
    # Determine the shared y-axis limit
    # Group by 'read', 'orientation', and 'whitelist', then apply value_counts on 'start'
    max_counts = barcode_data.groupby(['read', 'orientation', 'barcode_whitelist'])['start'].value_counts()
    max_count = max_counts.max()
    ylim = (0, max_count)

    # Create the 'forward' plot
    g1 = plot_facet(barcode_data[barcode_data['orientation'] == 'forward'], ylim = ylim)
    # Save the 'forward' plot temporarily
    g1.savefig('{}.{}'.format(outfile, 'f.png'), dpi = dpi)

    # Create the 'reverse' plot
    g2 = plot_facet(barcode_data[barcode_data['orientation'] == 'reverse'], ylim = ylim) 
    ...
    axes[0].axis('off')
    axes[0].set_title("Forward orientation", fontsize = 4, loc = 'left')

    axes[1].imshow(reverse_img)
    axes[1].axis('off')
    axes[1].set_title("Reverse orientation", fontsize = 4, loc = 'left')

    # Adjust layout and save the combined plot
    fig.tight_layout(pad=0)  # Minimize padding between subplots
 #   if save_fig:
    plt.savefig(outfile, dpi = dpi, bbox_inches='tight')  # Save with tight bounding box to remove white space
