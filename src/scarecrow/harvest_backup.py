...
class PeakAnalyzer:
    ...
    def find_peaks_optimized(self, 
        ...
        for p in peak_indices:
            ...
            peaks_with_details.append((start, end, peak_unique_names, read_fraction))
...
class HarvestOptimizer:
    ...
    def process_barcode_data(self, 
        ...
        for file in barcode_files:
            df = pd.read_csv(file, sep='\t', usecols=['read', 'name', 'barcode_whitelist', 'orientation', 'start', 'end'])
            dfs.append(df)
    ...
    def _get_barcode_peaks(self, barcode_data: pd.DataFrame) -> List[Dict]:
        ...
        for (read, barcode_whitelist, orientation), group in grouped:
            ...
            peaks = self.peak_analyzer.find_peaks_optimized(start_arr, end_arr, names_arr)
